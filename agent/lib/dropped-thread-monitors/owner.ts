...
import { z } from "zod";
import type { DroppedThreadMonitorOwner } from "@/db/services/dropped-thread-monitors";
import { scopeFromPrincipal } from "@/lib/access-scope";

const linqThreadIdSchema = z.string().regex(/^linq:/u);

export function requireDroppedThreadMonitorOwner(
  context: Pick<SessionContext, "session">
): DroppedThreadMonitorOwner {
  ...
  if (!linqThreadId.success) {
    throw new Error(
      "The dropped-thread monitor needs an iMessage destination. Text this assistant first, then configure the monitor from that conversation."
    );
  }
...
export function requireDroppedThreadMonitorScope(
  context: Pick<SessionContext, "session">
) {
  const caller = context.session.auth.current;
  if (caller?.principalType !== "user") {
