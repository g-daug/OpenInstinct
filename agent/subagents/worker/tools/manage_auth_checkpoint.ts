...
const inputSchema = z
  .object({
    action: z.enum(["pause", "complete", "fail"]),
    browser_session_id: z.string().trim().min(1).optional(),
    challenge_type: z.enum(browserAuthChallengeTypes).optional(),
    checkpoint_id: z.uuid().optional(),
